  ...
  <div class="min-h-screen relative">
    ...
    <div class="flex min-h-screen transition-all duration-300" :class="isMobile ? 'flex-col' : ''">
      ...
      <aside
        ...
        :class="[
          isMobile
            ...
            : 'translate-x-0',
          isMobile ? 'w-64' : (sidebarCollapsed ? 'w-[72px]' : 'w-[252px]')
        ]">
        <div
          class="flex h-full flex-col bg-white/76 backdrop-blur-2xl border-r border-black/[0.06] shadow-sm">
          <!-- Logo -->
          <div class="flex items-center gap-3 px-4 py-4 border-b border-black/[0.04]">
            <div
              class="flex h-[38px] w-[38px] shrink-0 items-center justify-center rounded-[11px] bg-gradient-to-br from-sky-500 to-indigo-500 text-white shadow-lg shadow-sky-500/25">
            ...
            <div class="overflow-hidden transition-all duration-300"
              ...
              <p class="text-[9px] text-slate-400 uppercase tracking-[0.15em] font-semibold leading-none">Platform</p>
              <p class="text-sm font-bold text-slate-800 leading-tight mt-0.5">BOPET EDA</p>
            </div>
          </div>
          ...
          <nav class="flex-1 overflow-y-auto px-2.5 py-3 space-y-0.5">
            <NuxtLink v-for="item in menuItems" :key="item.path" :to="item.path"
              ...
              class="group flex items-center gap-2.5 px-2.5 py-2 rounded-[10px] text-[13px] font-medium transition-all duration-200"
              :class="[
              ...
              <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-[8px] transition-all duration-200"
                :class="isMenuItemActive(item)
                  ? 'bg-gradient-to-br from-sky-500 to-indigo-500 text-white shadow shadow-sky-500/20'
                  : 'bg-black/[0.03] text-slate-400 group-hover:bg-black/[0.05] group-hover:text-slate-600'">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path v-for="(path, index) in getIconPaths(item.icon)" :key="index"
                    stroke-linecap="round" stroke-linejoin="round" stroke-width="1.7" :d="path" />
                </svg>
              </span>
          ...
          <div class="p-2.5 border-t border-black/[0.04]">
            <div
              class="flex items-center gap-2.5 p-2 rounded-[10px] bg-gradient-to-r from-slate-50/80 to-sky-50/40 border border-black/[0.04]"
              :class="sidebarCollapsed && !isMobile ? 'justify-center' : ''">
              <div
                class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-sky-500 to-indigo-500 text-white text-xs font-semibold shadow shadow-sky-500/20">
                {{ userInitial }}
              </div>
              <div class="overflow-hidden transition-all duration-300"
                :class="sidebarCollapsed && !isMobile ? 'w-0 opacity-0' : 'w-auto opacity-100'">
                <p class="text-xs font-semibold text-slate-700 truncate leading-tight">{{ authStore.user?.name || '当前用户' }}</p>
                <p class="text-[10px] text-slate-400 leading-tight">{{ roleLabel }}</p>
              </div>
              <button v-if="!sidebarCollapsed || isMobile"
                class="ml-auto p-1.5 rounded-lg text-slate-300 hover:text-rose-500 hover:bg-rose-50/60 transition-colors"
      ...
      <div class="flex-1 flex flex-col min-w-0">
        <!-- Header -->
        <header
          class="sticky top-0 z-30 bg-white/68 backdrop-blur-2xl border-b border-black/[0.05] px-5 sm:px-7 lg:px-9">
          <div class="flex items-center justify-between h-[58px]">
            <div class="flex items-center gap-3">
              <button
                class="flex h-9 w-9 items-center justify-center rounded-[9px] text-slate-400 hover:text-slate-700 hover:bg-black/[0.04] transition-colors lg:hidden"
                @click="toggleSidebar">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-[18px] w-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <button v-if="!isMobile"
                class="hidden lg:flex h-9 w-9 items-center justify-center rounded-[9px] text-slate-400 hover:text-slate-700 hover:bg-black/[0.04] transition-colors"
                @click="toggleSidebar">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-[18px] w-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M4 6h16M4 12h16M4 18h16" />
                </svg>
              ...
              <div>
                <p class="text-[9px] text-slate-400 uppercase tracking-[0.15em] font-semibold leading-none">{{ currentPageTitle }}</p>
                <p class="text-[11px] text-slate-400 leading-tight mt-0.5">{{ currentDate }}</p>
              </div>
            </div>

            <div class="flex items-center gap-3">
              <div
                class="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-sky-500 to-indigo-500 text-white text-xs font-semibold shadow shadow-sky-500/20">
                {{ userInitial }}
              </div>
            </div>
          </div>
...
const currentPageTitle = computed(() => {
  ...
  return menuItems.value.find(item => item.path === activeMenuPath.value)?.label || 'BOPET EDA'
})

const roleLabel = computed(() => {
  const map: Record<string, string> = { superadmin: '超级管理员', admin: '部门管理员', normal: '普通用户' }
...
const toggleSidebar = () => {
  if (isMobile.value) { mobileSidebarOpen.value = !mobileSidebarOpen.value; return }
...
onMounted(async () => {
  ...
  if (savedState !== null) sidebarCollapsed.value = savedState === 'true'
  updateViewport()
  window.addEventListener('resize', updateViewport)
})

onBeforeUnmount(() => { if (import.meta.client) window.removeEventListener('resize', updateViewport) })
watch(sidebarCollapsed, (value) => { if (import.meta.client) window.localStorage.setItem('layout-sidebar-collapsed', String(value)) })
