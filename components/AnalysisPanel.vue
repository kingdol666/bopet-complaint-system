<template>
  <div class="analysis-panel border rounded-xl bg-white shadow-sm overflow-hidden">
    ...
    <div v-if="loading" class="px-4 py-12 text-center"><n-spin size="medium" /><p class="text-xs text-gray-400 mt-2">正在分析数据...</p></div>

    <template v-if="result && !loading">
      <div class="grid grid-cols-4 gap-2 px-4 py-3 border-b bg-gray-50/50">
        <div class="text-center"><p class="text-xs text-gray-400">总计</p><p class="text-lg font-bold text-gray-800">{{ total }}</p></div>
        <div class="text-center"><p class="text-xs text-gray-400">分类数</p><p class="text-lg font-bold text-primary-600">{{ data.length }}</p></div>
        <div class="text-center"><p class="text-xs text-gray-400">最高</p><p class="text-sm font-semibold text-gray-700 truncate" :title="topItem?.name">{{ topItem?.name || '-' }}</p><p class="text-xs text-primary-600">{{ topItem?.count || 0 }}</p></div>
        <div class="text-center"><p class="text-xs text-gray-400">占比</p><p class="text-lg font-bold text-amber-600">{{ topItem?.percentage || '0' }}%</p></div>
      </div>

      <div class="px-4 py-3" style="height:280px">
        <ClientOnly><v-chart v-if="data.length" :option="chartOption" autoresize style="height:100%" /></ClientOnly>
      </div>
      ...
      <div class="px-4 pb-3">
        ...
        <n-data-table :columns="cols" :data="filteredData" size="small" :max-height="240" :bordered="false" :single-line="false" virtual-scroll />
      </div>

      <div class="px-4 pb-3 flex items-center justify-between">
        <div class="flex flex-wrap gap-1">
          <n-tag v-for="d in data.slice(0, 8)" :key="d.name" size="small" closable type="info" @click="filterBy(d.name)">{{ d.name }} ({{ d.count }})</n-tag>
        </div>
        <n-button size="tiny" quaternary type="primary" @click="$emit('save', currentConfig)">保存此分析</n-button>
      </div>
    </template>
...
const cols = [
  { title: '#', key: 'rank', width: 40 }, { title: '名称', key: 'name', ellipsis: { tooltip: true } },
  { title: '数量', key: 'count', width: 70 }, { title: '占比', key: 'percentage', width: 65 }
]
...
const chartOption = computed(() => {
  ...
  if (chartType.value === 'pie' || chartType.value === 'donut') {
    ...
    return {
      ...
      series: [{ type: 'pie', radius: innerRadius, center: ['38%', '50%'], itemStyle: { borderRadius: 4, borderColor: '#fff', borderWidth: 2 }, label: { show: false }, emphasis: { label: { show: true, fontSize: 12, fontWeight: 'bold' } }, data: d.map((x: any, i: number) => ({ name: x.name, value: x.count, itemStyle: { color: COLORS[i % 16] } })) }]
    }
  }
  if (chartType.value === 'hbar') {
    return {
      ...
      xAxis: { type: 'value' }, yAxis: { type: 'category', data: d.map((x: any) => x.name).reverse(), axisLabel: { width: 110, overflow: 'truncate', fontSize: 10 } },
  ...
  return {
    ...
    xAxis: { type: 'category', data: d.map((x: any) => x.name), axisLabel: { rotate: 35, width: 80, overflow: 'truncate', fontSize: 9 } },
    yAxis: { type: 'value' },
...
async function run() {
  ...
  try {
    ...
    if (resp.success) { data.value = (resp.data.results || []).map((d: any, i: number) => ({ ...d, rank: i + 1 })); total.value = resp.data.total; result.value = true }
  } catch (e: any) { message.error('分析失败') } finally { loading.value = false }
}
...
function exportCSV() {
  ...
  const rows = data.value.map((d: any) => `${d.rank},"${d.name}",${d.count},${d.percentage}`).join('\n')
  const blob = new Blob([BOM + header + rows], { type: 'text/csv;charset=utf-8' })
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a'); a.href = url; a.download = `${fLabel.value}_分析结果_${dayjs().format('YYYYMMDD')}.csv`; a.click()
