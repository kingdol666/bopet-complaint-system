<template>
  <div class="animate-fade-in">
    ...
    <!-- Template Edit Modal -->
    <n-modal v-model:show="modalVisible" preset="card" :title="editingId ? '编辑模板' : '新增模板'"
      style="width: 900px; max-height: 85vh; overflow-y: auto;">
      <n-form ref="formRef" :model="formData" :rules="formRules" label-placement="left" label-width="100">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <n-form-item label="所属部门" path="departmentId">
            <n-select v-model:value="formData.departmentId" :options="departmentOptions" placeholder="留空则为全局模板"
              clearable />
          </n-form-item>
        </div>
        ...
        <div class="border-t border-corporate-200 pt-4 mt-4">
          <div class="flex justify-between items-center mb-4">
            <div class="flex items-center gap-2">
              <div class="w-8 h-8 rounded-lg bg-primary-50 flex items-center justify-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-primary-600" fill="none" viewBox="0 0 24 24"
                  stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8"
          ...
          <div v-if="formData.fields.length === 0" class="text-center py-12">
            <div class="w-16 h-16 rounded-2xl bg-corporate-50 flex items-center justify-center mx-auto mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-corporate-400" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
          ...
          <div v-for="(field, index) in formData.fields" :key="index"
            class="mb-4 p-4 border border-corporate-200 rounded-xl bg-corporate-50/50">
            <div class="flex justify-between items-center mb-3">
              <div class="flex items-center gap-2">
                <div
                  class="w-6 h-6 rounded-md bg-primary-100 text-primary-700 text-xs font-semibold flex items-center justify-center">
                  {{ index + 1 }}
                </div>
