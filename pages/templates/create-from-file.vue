<template>
  <div class="animate-fade-in">
    <div class="flex items-center gap-4 mb-6">
      ...
      <div>
        <h1 class="page-title">智能创建模板</h1>
        <p class="page-subtitle">上传Excel/CSV文件，自动识别列名和类型，一键生成表单模板</p>
      </div>
    </div>
    ...
    <div class="card mb-6">
      ...
      <div
        class="border-2 border-dashed border-corporate-200 rounded-xl p-8 text-center hover:border-primary-400 transition-colors">
        <div class="w-16 h-16 rounded-2xl bg-primary-50 flex items-center justify-center mx-auto mb-4">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-primary-500" fill="none" viewBox="0 0 24 24"
            stroke="currentColor">
    ...
    <template v-if="parsed">
      <div class="card mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <n-form-item label="所属部门"><n-select v-model:value="config.departmentId" :options="deptOptions" clearable
              placeholder="全局" /></n-form-item>
          <n-form-item label="公开模板"><n-switch v-model:value="config.isPublic"
              :disabled="!config.departmentId" /></n-form-item>
        </div>
      </div>
      ...
      <div class="card mb-6">
        ...
        <div class="space-y-2">
          <div v-for="(f, i) in fields" :key="f.fieldKey" class="border rounded-lg overflow-hidden"
            :class="expandIdx === i ? 'border-primary-300 bg-primary-50/20' : 'border-corporate-200'">
            <div class="flex items-center justify-between px-4 py-2.5 cursor-pointer hover:bg-corporate-50"
              ...
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-corporate-400 transition-transform"
                :class="{ 'rotate-180': expandIdx === i }" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
              </svg>
            </div>
            <div v-if="expandIdx === i" class="px-4 py-3 border-t border-corporate-100 bg-white">
              <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mb-2">
                <n-form-item label="字段标签" :show-feedback="false"><n-input v-model:value="f.fieldLabel"
                    size="small" /></n-form-item>
                <n-form-item label="字段类型" :show-feedback="false">
                  <n-select v-model:value="f.fieldType" :options="typeOptions" size="small"
                    @update:value="() => onTypeChange(f)" />
                </n-form-item>
                <n-form-item label="字段键" :show-feedback="false"><n-input v-model:value="f.fieldKey" size="small"
                    disabled /></n-form-item>
              </div>
    ...
    <div v-if="result" class="card">
      ...
      <div class="grid grid-cols-3 gap-4 mb-4">
        <div class="bg-green-50 rounded p-3 text-center">
          <p class="text-2xl font-bold text-green-600">{{ result.template.id }}</p>
          <p class="text-xs text-green-500">模板ID</p>
        </div>
        <div class="bg-blue-50 rounded p-3 text-center">
          <p class="text-2xl font-bold text-blue-600">{{ fields.length }}</p>
          <p class="text-xs text-blue-500">字段数</p>
        </div>
        <div class="bg-amber-50 rounded p-3 text-center">
          <p class="text-2xl font-bold text-amber-600">{{ result.createdConfigs?.length || 0 }}</p>
          <p class="text-xs text-amber-500">下拉配置</p>
        </div>
      </div>
      <div class="flex gap-2">
